...
class Switched_trajectory
{
    ...
    Eigen::Vector3d e1_last,e2_last,e1,e2;
    double loopRate, e1_max,e2_max,e1_min,e2_min,lu,ls,epsilon,v_max,v_min;
    ...
    Eigen::Vector3d e1_reentry,e2_reentry;
	...
	public:
		Switched_trajectory()
		{
			...
			nhp.param<double>("e1_max",e1_max,1); 
			nhp.param<double>("e2_max",e2_max,1); 
			nhp.param<double>("e1_min",e1_min,0.1); 
			nhp.param<double>("e2_min",e2_min,0.1);
			...
			v_max = (1.0/2)*(pow(e1_max,2)+pow(e2_max,2));
			v_min = (1.0/2)*(pow(e1_min,2)+pow(e2_min,2));
			ls = 2*(*min_element(k1.begin(),k1.end()));
			epsilon = (1.0/2)*sqrt(dmax.at(0)*dmax.at(0)+dmax.at(1)*dmax.at(1));
			stage = 0;
			feedback = true;
			init = false;
			pp = false;
			...
			switchedPose.pose.position.x = 1-e1_max-e2_max;
		...
		void mocapCB(const geometry_msgs::PoseStampedPtr& pose)
		{
			...
			if(pp && pt && sp && st && bp && bt && pdp)
			{
				...
				e1 << predictorPose.pose.position.x - switchedPose.pose.position.x, predictorPose.pose.position.y - switchedPose.pose.position.y, yawp - yawd; 
				e2 << bebopPose.pose.position.x-predictorPose.pose.position.x, bebopPose.pose.position.y-predictorPose.pose.position.y, yaw - yawp;
				
				// make sure theta tilde is between 0 and 2*pi
				double theta_tilde1 = fmod(e1(2),2.0*M_PI);
				double theta_tilde2 = fmod(e2(2),2.0*M_PI);
				...
				if(!init)
				{
					e1_last = e1;
					e2_last = e2;
					double v_sigma = (1.0/2.0)*(e1_last(0)*e1_last(0) + e1_last(1)*e1_last(1) + e2_last(0)*e2_last(0) + e2_last(1)*e2_last(1));
		...
		void switchedCB(const ros::TimerEvent& )
		{
			...
			if(stage == 0 && time_stage > dwell_time_min)
			{
				...
				// linear
				double ep1 = epsilon;
				double ep2 = sqrt(dmax.at(0)*dmax.at(0)+dmax.at(1)*dmax.at(1))*sqrt(e2_last(0)*e2_last(0) + e2_last(1)*e2_last(1));
			...
			else if(stage == 1 && feedback && time_stage > dwell_time_max)
			{
				...
				double v_sigma = (1.0/2.0)*(e1_reentry(0)*e1_reentry(0) + e1_reentry(1)*e1_reentry(1) + e2_reentry(0)*e2_reentry(0) + e2_reentry(1)*e2_reentry(1));
				dwell_time_min = -(1.0/ls)*log((v_min)/(v_sigma));
