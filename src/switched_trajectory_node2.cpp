...
class Switched_trajectory
{
    ...
    geometry_msgs::Twist primaryTwist,secondaryTwist,boundaryTwist,switchedTwist,bebopCurTwist;
	geometry_msgs::PoseStamped primaryPose,secondaryPose,boundaryPose,switchedPose,bebopPose, predictorPose, bebopCurPose;
    ...
    bool recordE,feedback,pp,pt,sp,st,bp,bt,pdp,dataSaved,advancePath,instantSwitch,flag;
	...
	public:
		Switched_trajectory()
		{
			...
			nhp.param<bool>("instantSwitch",instantSwitch,true);
		...
		void switchedCB(const ros::TimerEvent& )
		{
			...
			else 
			{
				if(stage == 0 && time_stage > dwell_time_min)
				{
					bebopCurPose = bebopPose;
					bebopCurTwist.linear.x = 0;
					bebopCurTwist.linear.y = 0;
					bebopCurTwist.linear.z = 0;
					bebopCurTwist.angular.z = 0;
					
					tf::Quaternion bcp;
					bcp.setX(bebopCurPose.pose.orientation.x);
					bcp.setY(bebopCurPose.pose.orientation.y);
					bcp.setZ(bebopCurPose.pose.orientation.z);
					bcp.setW(bebopCurPose.pose.orientation.w);
					
					tf::Quaternion pp;
					pp.setX(primaryPose.pose.orientation.x);
					pp.setY(primaryPose.pose.orientation.y);
					pp.setZ(primaryPose.pose.orientation.z);
					pp.setW(primaryPose.pose.orientation.w);					
					
					if ((bcp*pp.inverse()).getW() < 0)//check if it flipped
					{
						bebopCurPose.pose.orientation.x *= -1.0;
						bebopCurPose.pose.orientation.y *= -1.0;
						bebopCurPose.pose.orientation.z *= -1.0;
						bebopCurPose.pose.orientation.w *= -1.0;
					}
					e << 0,0,0;
					eHat << 0,0,0;
					zeta << 0,0,0;
				...
				else if (stage == 1 && time_stage > 0.5*dwell_time_max && feedback)
				{
					bebopCurPose = bebopPose;
					bebopCurTwist.linear.x = 0;
					bebopCurTwist.linear.y = 0;
			...
			if( stage == 0 )
			{
				...
				{		//std::cout << time_stage << std::endl;
						switchedPose.pose.position.x = bebopPose.pose.position.x;
						switchedPose.pose.position.y = bebopPose.pose.position.y;
						switchedPose.pose.position.z = bebopPose.pose.position.z;
						switchedPose.pose.orientation.z = bebopPose.pose.orientation.z;
						switchedPose.pose.orientation.w = bebopPose.pose.orientation.w;
			...
			else
			{
				if ( instantSwitch )
				{
					...
					if(ratio <= phase0)
					{
						...
						switchedPose.pose.position.x = ss*primaryPose.pose.position.x+(1-ss)*bebopCurPose.pose.position.x;
						switchedPose.pose.position.y = ss*primaryPose.pose.position.y+(1-ss)*bebopCurPose.pose.position.y;
						switchedPose.pose.position.z = ss*primaryPose.pose.position.z+(1-ss)*bebopCurPose.pose.position.z;
						switchedPose.pose.orientation.z = ss*primaryPose.pose.orientation.z+(1-ss)*bebopCurPose.pose.orientation.z;
						switchedPose.pose.orientation.w = ss*primaryPose.pose.orientation.w+(1-ss)*bebopCurPose.pose.orientation.w;
					
						switchedTwist.linear.x = ssDot*primaryPose.pose.position.x + ss*primaryTwist.linear.x - ssDot*bebopCurPose.pose.position.x + (1-ss)*bebopCurTwist.linear.x;
						switchedTwist.linear.y = ssDot*primaryPose.pose.position.y + ss*primaryTwist.linear.y - ssDot*bebopCurPose.pose.position.y + (1-ss)*bebopCurTwist.linear.y;
						switchedTwist.linear.z = ssDot*primaryPose.pose.position.z + ss*primaryTwist.linear.z - ssDot*bebopCurPose.pose.position.z + (1-ss)*bebopCurTwist.linear.z;
						switchedTwist.angular.z = ssDot*primaryPose.pose.orientation.z + ss*primaryTwist.angular.z - ssDot*bebopCurPose.pose.orientation.z + (1-ss)*bebopCurTwist.angular.z;
