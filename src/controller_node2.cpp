...
class Predictor
{
    ...
    Eigen::Vector3d statesP,statesHatP,statesDesP,statesHatL,statesHatA,desL,desA,e1P,e2P,eP,rL,rA,uL,uA;
    Eigen::Vector4d statesQ,statesHatQ,statesDesQ,lastQ,e1Q,e2Q,eQ;
    
    DerivativeEstimator hatQdot;
    std::vector<double> k1_gain,k2_gain,d_max;
    bool sp,st,init,feedback,mocap,dataSaved,instantSwitch;
	...
	public:
		...
		void mocapCB(const geometry_msgs::PoseStampedPtr& pose)
		{
			...
			if ((statesQ-(-1.0*statesDesQ)).norm() < (statesQ-statesDesQ).norm())//check if it flipped
			{
				statesQ *= -1.0;
			}
			...
			tf::Quaternion eQtf = truQ*desQ.inverse();
			tf::Quaternion e1Qtf = hatQ*desQ.inverse(); 
			tf::Quaternion e2Qtf = truQ*hatQ.inverse();
			
			eQ << eQtf.getW(),eQtf.getX(),eQtf.getY(),eQtf.getZ();
			...
			if (feedback)
			{
				uL = -k1.cwiseProduct(eP) + swL - rL;
				uA = -2.0*k1.cwiseProduct(eQ.segment(1,3)) + swA - rA;
			}
		...
		void predictorCB(const ros::TimerEvent& )
		{
			...
			if(feedback) statesHatL = uLa_world.segment(1,3)+rL;
			else statesHatL = uL+rL;
